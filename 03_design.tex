...
fewest number of matching events, one might choose to collect and abstract over 
the symbolic sets of those transitions.
Due to their low cardinality these sets are likely to have very high 
filtering and the data abstractions used to enforce them can achieve higher
precision for the same operating costs (considering the small number of values 
to store and query).
Applying this heuristic to our example from section\ref{sec:mot_example} wrt.\ 
to the final transition P produces the following relaxed filters:
\begin{align*}
content...
\end{align*}
...
We remark that the abstract filters are not strictly tied to the ability to 
