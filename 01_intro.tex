...
Warehoused data is typically processed by a mix of workloads.  For
...
{\small
	...
	AND NOT EXISTS ( 
	SELECT * FROM Ev NR
	WHERE S.time < NR.time AND NR.time < P.time
	AND NR.user == S.user
	AND NR.name != "R" ); 
	\end{verbatim}
}
...
\}
\end{align*}
We introduce a temporal slicing operator that simplifies our
notation. Given a set $S$ of tuples $\tuple{s_1,s_2,\ldots,s_n}$ and
unary predications $\phi_1, \phi_2, \ldots, \phi_n$, we define the
slicing operator:
...
Now, We re-write the query, first using comprehension syntax, 
...
&\equiv
\exists \symv{s} \in 
\slice{\syms{S}}{(-\infty\,:\,\facc{\symv{r}}{t})_\in, \facc{\symv{r}}{user}_=},
\\
&\quad
\exists \symv{p} \in 
\slice{\syms{P}}
{(\max(\facc{\symv{s}}{t},\facc{\symv{r}}{t})\,:\,\facc{\symv{s}}{t} + t_{out})_\in,
	\facc{\symv{r}}{user}_=}
: 
\\ 
&\qquad\quad
\slice{\syms{\oR}}
{(\facc{\symv{s}}{t}\,:\,\facc{\symv{p}}{t})_\in,\; \facc{\symv{r}}{user}_=} 
= \emptyset  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\\
\precs{P}(\symv{p}) 
